...
import { transformPSPDFKitRect, transformPSPDFKitQuadPoints, transformPSPDFKitLineCoordinates } from './ui_utils'
...
class AnnotationLayerProxy {
  ...
  _caculateRect (page, coord) {
    ...
    return this._pageCoordinateToPDFPoint(viewport, coord.tx, coord.ty, coord.bx, coord.by)
  }

  _getRandomString () {
    return parseInt(Date.now() + '' + Math.ceil(Math.random().toFixed(15) * 1e16)).toString(16)
  }
  ...
  _pageCoordinateToPDFPoint (viewport, fx, fy, lx, ly) {
    ...
    const [buttomRightX, bottomRightY] = this._convertToPDFPoint(viewport, lx, ly)
  ...
  _convertRectToAnnotation (annotation) {
    ...
    if (annotation.type && annotation.type.startsWith('pspdfkit')) {
      ...
      if (quadPoints) {
        _annotation.set('quadPoints', transformPSPDFKitQuadPoints(params.viewport, quadPoints))
      }
      if (lineCoordinates) {
        const _lineCoordinates = transformPSPDFKitLineCoordinates(params.viewport, lineCoordinates)
