...
class SelectHandler {
  ...
  drawPoints () {
    ...
    for (let i = 0, len = array.length; i < len; ++i) {
      ...
      let point = this.drawPoint(array[i][0], array[i][1]).addClass(this.options.classPoints).addClass(this.options.classPoints + '_point').on('touchstart', curriedEvent).on('mousedown', curriedEvent)
  ...
  selectRect (value) {
    ...
    if (value) {
      this.el.style('pointer-events', 'fill')
    } else {
      this.el.style('pointer-events', 'painted')
    }
    ...
    if (!this.rectSelection.set.get(0)) {
      this.rectSelection.set.add(this.nested.rect(bbox.width, bbox.height).addClass(this.options.classRect).style('pointer-events', 'none'))
    }

    // Draw Points at the edges, if enabled
    if (this.options.points.length && this.rectSelection.set.length() < 2) {
      let ename = 'touchstart'
      let mname = 'mousedown'

      this.options.points.map(function (point, index) {
        let coords = _this.pointCoords(point, bbox)

        let pointElement = _this.drawPoint(coords.x, coords.y).attr('class', _this.options.classPoints + '_' + point).on(mname, getMoseDownFunc(point)).on(ename, getMoseDownFunc(point))
    ...
    if (this.options.rotationPoint && ((this.options.points && !this.rectSelection.set.get(9)) || (!this.options.points && !this.rectSelection.set.get(1)))) {
      ...
      let pointElement = this.drawPoint(bbox.width / 2, 20).attr('class', this.options.classPoints + '_rot').on('touchstart', curriedEvent).on('mousedown', curriedEvent)
...
SVG.Element.prototype.selectize.defaults = {
  ...
  pointFill: '#3BB8AF',                       // Point fill color
  pointStroke: { width: 1, color: '#3BB8AF' } // Point stroke properties
}
